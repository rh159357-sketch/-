...
export default function App() {
  ...
  useEffect(() => {
    try {
      ...
      // 1) addMonths 말일 롤오버
      console.assert(addMonths("2025-01-31", 1).startsWith("2025-02"), "addMonths rollover");
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900">
      <header className="sticky top-0 z-10 bg-white border-b">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="text-xl font-bold">보장구 대여 — 실시간 현황 & 신청/반납</div>
        </div>
      </header>
...
function RentalForm({ items, onSubmit }) {
  ...
  return (
    <Card title="대여 신청">
      ...
      <div className="text-sm bg-gray-50 rounded-xl p-3 mt-3">예상 반납일: <b>{dueDate}</b> (최대 {maxMonths}개월)</div>
  ...
  return (
    <Card title="기능 별 목록">
      <ul className="list-disc ml-5 space-y-1 text-sm">
        ...
        <li>반납 관리 – 전체 이력(정렬/개별 삭제/전체 삭제)</li>
